...
from transformers.models import TimmWrapperModel
...
class Gemma3nModel(Gemma3nPreTrainedModel):
    ...
    def __init__(self, config: Gemma3nConfig):
        super().__init__(config)
        self.vision_tower = TimmWrapperModel(config=config.vision_config) # AutoModel.from_config(config=config.vision_config)
        self.vocab_size = config.text_config.vocab_size

        language_model = Gemma3nTextModel(config=config.text_config) # AutoModel.from_config(config=config.text_config)
        self.language_model = language_model

        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.vocab_size_per_layer_input = config.text_config.vocab_size_per_layer_input
        self.audio_tower = Gemma3nAudioEncoder(config.audio_config) # AutoModel.from_config(config.audio_config)
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        if input_ids is not None:
            ...
            vision_embeds = self.embed_vision(input_ids=vision_input_ids).to(inputs_embeds.device)
            expanded_vision_mask = vision_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            ...
            audio_embeds = self.embed_audio(input_ids=audio_input_ids).to(inputs_embeds.device)
            expanded_audio_mask = audio_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
