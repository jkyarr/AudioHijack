...
import util
from metrics import StringMatch, BehaviorMatchTool
...
class Attack:
    ...
    def __call__(self, audio_data, user_prompts, history, label, wav_dir):
        ...
        util.save_audio(
            self.ben_audio.cpu(),
            os.path.join(wav_dir, f'ben_{self.carrier}_len_{self.carrier_length}.wav')
        )
        self.scheduler = torch.optim.lr_scheduler.LinearLR(
            self.optimizer, start_factor=1.0, end_factor=0.1, total_iters=self.steps
        )
        if self.accum_grad:
        ...
        with torch.no_grad():
            for i in range(0, len(user_prompts), batch_size):
                ...
                if 'Tool Misuse' in behavior:
                    ...
                    call_success_list += [BehaviorMatchTool.check_success(behavior, tool_call) for tool_call in tool_calls]
                    adv_resp_list += ['\n'.join(response) for response in responses]
                else:
                    _, responses = model.generate(messages)
                    adv_resp_list += responses
        success_list = [StringMatch.check_success(label, response) for response in adv_resp_list]
